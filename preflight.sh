...
if [ -d /usr/local/cuda/lib64 ]; then ok 'CUDA runtime directory'; else bad '/usr/local/cuda/lib64'; fi
...
required_files=(
  "$BUNDLE_ROOT/threefloor_stress5_repro_20260827/run_native.sh"
  "$BUNDLE_ROOT/SimEnv-master/src/unitree_guide/unitree_guide/unitree_guide/CMakeLists.txt"
  "$BUNDLE_ROOT/threefloor_stress5_repro_20260827/work/mounts/SCAN-Planner/src/planner/plan_manage/package.xml"
...
for required in "${required_files[@]}"; do
  if [ -f "$required" ]; then ok "bundle file: ${required#"$BUNDLE_ROOT/"}"; else bad "bundle file: ${required#"$BUNDLE_ROOT/"}"; fi
done
